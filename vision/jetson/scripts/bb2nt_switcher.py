...
NetworkTables.initialize() # this is a server, if client
#uncomment this: (server='tegra-ubuntu.local')

table = NetworkTables.getTable('vision')
print('got network table: ' + table.path)
current_model = 'box'
	...
	for line in iter(sys.stdin.readline, ''):
		if 'camera open for streaming' in line:
			...
			os.system('v4l2-ctl -d /dev/video1 -c exposure_auto=1\n'\
					'v4l2-ctl -d /dev/video1 -c exposure_absolute=20\n' + \
					'v4l2-ctl -d /dev/video1 -c brightness=' + \
					box_brightness + \
					'\nv4l2-ctl -l')
		...
		model = table.getString("model", current_model)
